...
{
	 //nu pot pune dim-1, deoarece e adresa-1, eu trebuie sa iau valoarea, adica folosesc DEREFERENTIERE, adica iau valoarea din adresa, deci *dim-1
	int* vectorCopie = new int[*dim - 1];
	int j=0,i;
	if (poz < 0 || poz >= *dim)
	{
		cout << endl << "Pozitie invalida";
		return;
	}
	...
	delete[] *vector;
	//dezaloc vectorul ca nu mai am nevoie de el
